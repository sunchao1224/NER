...
class AdapterBiLSTM(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(AdapterBiLSTM, self).__init__()

        self.input_dim = input_dim
        self.hidden_dim = 200
        self.output_dim = output_dim

        self.adapter_lstm = nn.LSTM(input_dim, self.hidden_dim, bidirectional=True)
        self.adapter_linear = nn.Linear(self.hidden_dim * 2, output_dim)

        # Initialize LSTM
        nn.init.xavier_uniform_(self.adapter_lstm.weight_ih_l0)
        nn.init.xavier_uniform_(self.adapter_lstm.weight_hh_l0)
        nn.init.xavier_uniform_(self.adapter_lstm.weight_ih_l0_reverse)
        nn.init.xavier_uniform_(self.adapter_lstm.weight_hh_l0_reverse)
        self.adapter_lstm.bias_ih_l0.data.fill_(0)
        self.adapter_lstm.bias_hh_l0.data.fill_(0)
        self.adapter_lstm.bias_ih_l0_reverse.data.fill_(0)
        self.adapter_lstm.bias_hh_l0_reverse.data.fill_(0)

    def forward(self, input_feats):
        adapter_lstm_out, _ = self.adapter_lstm(input_feats)
        adapter_out = self.adapter_linear(adapter_lstm_out)
        return adapter_out
...
class BiLSTM_CRF(nn.Module):
    def __init__(
        ...
        self.device = torch.device("cuda:4" if self.use_gpu else "cpu")
        ...
        # neuron adapter
        # self.adapter_bilstm = AdapterBiLSTM(self.tagset_size, self.tagset_size)
    ...
    def _get_lstm_features(self, sentence, chars, caps, chars2_length, d):
        ...
        # Apply the adapter BiLSTM on the output of the fully connected layer
        # lstm_feats = self.adapter_bilstm(lstm_feats)
        return lstm_feats
