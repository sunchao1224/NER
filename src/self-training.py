...
optparser.add_option("-T", "--train", default="data/E-NER-Dataset-main/E-NER_train.txt", help="Train set location")
optparser.add_option("-d", "--dev", default="data/E-NER-Dataset-main/E-NER_dev.txt", help="Dev set location")
optparser.add_option("-t", "--test", default="data/E-NER-Dataset-main/E-NER_test.txt", help="Test set location")
optparser.add_option("--soft", default="data/E-NER-Dataset-main/softlabels.txt", help="softlabels set location")
optparser.add_option("--test_train", default="data/E-NER-Dataset-main/E-NER_train_10000_test.txt", help="test train")
optparser.add_option("--score", default="evaluation/temp/scoreENER.txt", help="score file location")
...
optparser.add_option("-r", "--reload", default="1", type="int", help="Reload the last saved model")
...
optparser.add_option("--name", default="model_10k_train_ENER_selftraining", help="model name")
optparser.add_option("--char_mode", choices=["CNN", "LSTM"], default="CNN", help="char_CNN or char_LSTM")
opts = optparser.parse_args()[0]

parameters = OrderedDict()
parameters["soft"] = opts.soft
...
mapping_file = "models/mapping_" + name + ".pkl"
models_path = "models/"
model_name = models_path + name  # get_name(parameters)
tmp_model = model_name + ".tmp"

def softlabelling(model, datas):
    softlabels = []
    ...
    for data in tqdm(datas):
        ...
        for (word, pred_id) in zip(words, predicted_id):
            ...
            softlabels.append(line)
        softlabels.append('')
    
    softlabels_path = "data/E-NER-Dataset-main/softlabels.txt"

    with open(softlabels_path, 'w') as f:
        f.write('\n'.join(softlabels))
...
def train(train_data):
    ...
    eval_every = 200
    count = 0
    vis = visdom.Visdom(server='http://10.2.56.213', port=8097)
    sys.stdout.flush()

    model.train(True)

    for epoch in range(1, 3):
        softlabelling(model, train_data)
        train_sentences = loader.load_sentences(opts.soft, lower, zeros)
...
if __name__ == "__main__":
    ...
    if parameters["reload"]:
        ...
        model.char_embeds = model0.char_embeds
        model.word_embeds = model0.word_embeds
    else:
        model = model0
